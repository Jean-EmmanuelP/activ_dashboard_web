<script lang="ts">
	...
	let isQuestionnaireActive = $derived(
		isInSubmission && 
		submissionId && 
		currentPath.includes(`/submission/${submissionId}`) && 
		!currentView.includes('prescription') && 
		!currentView.includes('conseils')
	);
	
	let isPrescriptionActive = $derived(
		isInSubmission && 
		submissionId && 
		currentView.includes('prescription')
	);
	
	let isConseilsActive = $derived(
		isInSubmission && 
		submissionId && 
		currentView.includes('conseils')
	);
	...
	onMount(async () => {
		...
		// Track current path and view for active state
		...
		<div class="flex flex-col h-full">
			...
			<div class="p-6 border-b border-gray-200">
				...
				<div class="mt-4 p-3 rounded-lg bg-gray-50">
					...
					<p class="font-normal text-xs mt-1 text-gray-600">
						{user.email}
					</p>
				</div>
			</div>
			...
			<nav class="flex-1 p-4">
				<div class="space-y-2">
					<a 
						href="/dashboard" 
						class="flex items-center gap-3 py-3.5 px-4 rounded-lg transition-all duration-200 {currentPath === '/dashboard' ? 'bg-blue-900 text-white shadow-lg' : 'text-gray-700 hover:bg-gray-100'}"
					>
					...
					{#if isInSubmission && submissionId}
						<div class="ml-4 pl-4 border-l-2 border-gray-300">
							<a 
								...
								class="flex items-center gap-3 text-sm py-2.5 px-3.5 rounded-lg transition-all duration-200 {isQuestionnaireActive ? 'bg-blue-900 text-white shadow-lg' : 'text-gray-700 hover:bg-gray-100'}"
							>
							...
							{#if hasAIRecommendations}
								<div class="mt-2 pl-2">
									<div class="text-xs text-gray-500 mb-1.5 font-semibold">
										Recommandations IA
									</div>
									<a 
										href="/dashboard/submission/{submissionId}?view=prescription" 
										class="flex items-center gap-3 text-sm py-2 px-3 rounded-lg transition-all duration-200 {isPrescriptionActive ? 'text-black font-bold underline decoration-2 underline-offset-4' : 'text-gray-700 hover:bg-gray-100'}"
									>
										...
										<span class="font-medium">Prescription Médicale</span>
									</a>
									
									<a 
										href="/dashboard/submission/{submissionId}?view=conseils" 
										class="flex items-center gap-3 text-sm py-2 px-3 rounded-lg transition-all duration-200 {isConseilsActive ? 'text-black font-bold underline decoration-2 underline-offset-4' : 'text-gray-700 hover:bg-gray-100'}"
									>
										<svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
											<path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
										</svg>
										<span class="font-medium">Conseils APA</span>
									</a>
								</div>
							{:else}
								<div class="mt-2 pl-2">
					...
					<a 
						...
						class="flex items-center gap-3 py-3.5 px-4 rounded-lg transition-all duration-200 {currentPath === '/dashboard/settings' ? 'bg-blue-900 text-white shadow-lg' : 'text-gray-700 hover:bg-gray-100'}"
					>
						<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
			...
			<div class="p-4 border-t border-gray-200">
				<button 
					...
					class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition-colors duration-200"
				>
